...
async function fetchSalesOrderPdfFromOdoo(
  ...
  console.log(`API: Attempting to fetch Sales Order PDF for user input: ${soUserInputName}`);
  ...
  try {
    ...
    console.log(`API: Odoo XML-RPC authentication successful for SO fetch. UID: ${uid}`);
  } catch (authError: any) {
    console.error('API: Odoo XML-RPC Authentication Error (SO Fetch):', authError.message);
  ...
  console.log(`API: Found Sales Order: ${actualSaleOrderName} (ID: ${saleId})`);
  ...
  try {
    console.log('API: Attempting Odoo HTTP login for SO PDF download...');
    ...
    if (loginResponse.status !== 200 || !loginResponse.data.result) {
      console.error('API: Odoo HTTP Login Failed. Status:', loginResponse.status, 'Response Data:', loginResponse.data);
    ...
    console.log('API: Odoo HTTP login successful for SO PDF download.');
  } catch (loginErr: any) {
    console.error('API: Odoo HTTP Login Request Error:', loginErr.response?.status, loginErr.response?.data, loginErr.message);
  ...
  console.log(`API: Attempting to download SO PDF from: ${reportUrl}`);
  ...
  console.log(`API: SO PDF Download Response Status: ${pdfResponse.status}, Content-Type: ${contentType}`);
  ...
  if (pdfResponse.status !== 200 || !contentType || !contentType.toLowerCase().includes('application/pdf') || !pdfResponse.data || !(pdfResponse.data.byteLength > 0)) {
    ...
    console.error('API:', errorDetails, 'Data length:', pdfResponse.data?.byteLength);
    throw new Error(errorDetails);
  }
  console.log(`API: Successfully fetched SO PDF: ${actualSaleOrderName}.pdf, Size: ${pdfResponse.data.byteLength} bytes`);
...
async function fetchLinkedPurchaseOrderPdfFromOdoo(
  ...
  console.log(`API: Attempting to fetch linked Purchase Order PDF for SO: ${originalSoName}`);
  const modelsClient = xmlrpc.createSecureClient(`${odooUrl}/xmlrpc/2/object`);
  ...
  console.log(`API: Found linked Purchase Order: ${poName} (ID: ${poId})`);
  ...
  console.log(`API: Found PO PDF attachment: ${attachmentName}`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`API: Fetching Sales Order PDF for user input: ${salesOrderUserInputName.trim()}`);
    salesOrderDetails = await fetchSalesOrderPdfFromOdoo(salesOrderUserInputName.trim(), odooUrl, odooDb, odooUsername, odooPassword);
    console.log(`API: Successfully fetched Sales Order PDF: ${salesOrderDetails.fileName}, Original SO Name: ${salesOrderDetails.originalName}, Size: ${salesOrderDetails.size} bytes`);
    ...
    try {
        ...
        console.log(`API: Odoo XML-RPC authentication successful for PO fetch. UID: ${poFetchUid}`);
    } catch (authError: any) {
        console.error('API: Odoo XML-RPC Authentication Error (PO Fetch):', authError.message);
        throw new Error(`Odoo Authentication Failed (XML-RPC for PO): ${authError.message}. Please check server credentials for Odoo.`);
    }

    console.log(`API: Fetching linked Purchase Order PDF using original SO name: ${salesOrderDetails.originalName}`);
    ...
    console.log(`API: Successfully fetched linked Purchase Order PDF: ${purchaseOrderDetails.fileName}, Original PO Name: ${purchaseOrderDetails.originalName}, Size: ${purchaseOrderDetails.size} bytes`);
    ...
    if (salesOrderDetails.size === 0 || purchaseOrderDetails.size === 0) {
        ...
        console.warn("API: " + warningMessage + "This will likely cause issues with AI comparison.");
    }
